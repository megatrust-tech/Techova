...
public interface ILeaveService
{
    ...
    Task<PagedResult<UserWithoutBalanceDto>> GetUsersWithoutBalancesAsync(PaginationDto pagination);
    Task<InitializeBalancesResponseDto> InitializeBalancesAsync(InitializeBalancesRequestDto request);
    Task<PagedResult<UserBalanceDto>> GetAllUsersWithBalancesAsync(PaginationDto pagination);
...
public class LeaveService : ILeaveService
{
    ...
    public async Task<PagedResult<UserWithoutBalanceDto>> GetUsersWithoutBalancesAsync(PaginationDto pagination)
    {
        var currentYear = DateTime.UtcNow.Year;
    ...
    public async Task<InitializeBalancesResponseDto> InitializeBalancesAsync(InitializeBalancesRequestDto request)
    {
        ...
        var existingBalances = await _context.LeaveBalances
            .Where(b => userIds.Contains(b.EmployeeId) && b.Year == currentYear)
        ...
        return new InitializeBalancesResponseDto
        {
            Message = $"Leave balances initialized for {userIds.Count} users ({balancesToAdd.Count} balance records created)",
            Count = userIds.Count
        };
    }

    public async Task<PagedResult<UserBalanceDto>> GetAllUsersWithBalancesAsync(PaginationDto pagination)
    {
        var currentYear = DateTime.UtcNow.Year;
        var allLeaveTypes = Enum.GetValues(typeof(LeaveType)).Cast<LeaveType>().ToList();
    ...
    public async Task<UpdateBalancesResponseDto> UpdateBalancesAsync(UpdateBalancesRequestDto request)
    {
        ...
        return new UpdateBalancesResponseDto
        {
            Message = $"Updated {recordsUpdated} balance records, created {recordsCreated} new records for {userIds.Count} users",
