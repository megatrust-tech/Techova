...
public class LeaveServiceTests
{
    ...
    [Fact]
    public async Task GetPendingApprovalCount_HR_ReturnsAllPendingHR()
    {
        // Arrange
        int hrId = 9050;
        await SeedUser(hrId, null, roleId: 3);

        // Seed PendingHR requests
        await SeedRequest(9060, 999, LeaveStatus.PendingHR, DateTime.UtcNow, DateTime.UtcNow.AddDays(1), 1);
        await SeedRequest(9061, 999, LeaveStatus.PendingHR, DateTime.UtcNow, DateTime.UtcNow.AddDays(2), 1);
        await SeedRequest(9062, 999, LeaveStatus.PendingHR, DateTime.UtcNow, DateTime.UtcNow.AddDays(3), 1);
